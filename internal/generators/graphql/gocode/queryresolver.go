...
func (r *Resolver) {{title .Query.Name}}(ctx context.Context, in *{{title .Query.Name -}}Input) (*{{.ReturnType.ConnectionName}}, error) {
	...
	data, hasNextPage, hasPreviousPage, err :=  r.{{- .ReturnType.Module -}}Store.Get{{- plural (title .ReturnType.Name)}} (
		ctx,
		{{- template "getargs" .}}
	)
	if err != nil {
	...
	return New{{- .ReturnType.ConnectionName}}(data, hasNextPage, hasPreviousPage)
}
